...
- Never accept `watch start` with `proc=unseen` as a successful attachment. Remove that named watch while the launch gate is still closed; `proc=unseen`, named-registry capacity exhaustion, and `exact PID ... is not running` for a newly spawned gated process may switch the unopened payload once to an exact-PID wait only after its terminal request is connected. Never retry a provider command after it may have executed, including after terminal verification, ack, or cleanup failure.
...
- The external memory layer uses the latest World Memory report summary with `월드 메모리 변경 제안` removed, plus a translation-model market summary of News Feed items after `data/world-memory/collector-state.json` `collector.lastSuccessfulAt`. A new World Memory cutoff creates a bounded 30-60 item baseline. Later refreshes keep the previous summary as context and apply new items only after six accumulate or 30 minutes elapse. The same model response includes semantic severity fields (`alertLevel`, `severityKo`, `shouldNotify`, `pushSummary`), so severity is not a second model pass. `urgent` and `critical` queue a deduplicated browser notification that opens `News Feed`; they never generate an emergency report or make `보고서` red. Exact semantic inputs reuse the successful persisted summary, concurrent refreshes share one runtime-file-lease single flight, and raw News Feed lists are never appended to prompt memory.
...
- Every new magazine article must include the selected structured World Memory event ids plus semantic-search/vector evidence in `metadata.json`. If supporting semantic hits are sparse or noisy, keep the selected World Memory angle locked and use external research to verify or deepen it. If no structured World Memory candidate supports an article-worthy angle, skip the slot instead of replacing the subject.
...
- Before creating a magazine issue, build an editorial slate that avoids duplicate generic explainers. Recurring mega-trends should be written as follow-ups focused on the latest delta, while each issue should also mix lower-level signals, company/sector mechanics, and World Memory angles that benefit from deeper official or external verification. Use `editorialAngle`, `storyFamily`, and `noveltyNote` in metadata when possible.
- Every new Magazine article angle must be selected from structured World Memory candidates. News Feed items and web-search results may verify or enrich a selected angle, but must never introduce or replace the subject. Staged generation must compare World Memory candidates against recent uploaded articles before publish. Reusing the same primary World Memory event id is continuity context, not a standalone veto; it becomes a failure when the article also stays in the same `storyFamily` without a new World Memory event, state change, number, policy execution, price reaction, company action, or verified official/external source anchor. Independent delta is not whole-article embedding distance and not a changed title, image, or surface `storyFamily`. New articles should include `metadata.eventSignature` as a compact primary claimlet (`role`, `actor`, `action`, `object[]`, `time`, `marketMechanism`, `sourceIds[]`), or `metadata.eventSignatures[]` with exactly one primary card plus optional supporting cards. Novelty embedding should use the primary claimlet plus source titles and `noveltyNote`, not the article body. Ambiguous cases should be judged by an LLM novelty classification of `same_event`, `independent_followup`, or `unrelated`.
